...
class AffectGPT(Blip2Base):
    ...
    '''
    def prepare_inputs_embeds(self, samples):
        """
        把多模态特征 (face/audio/multi 等) 替换进 prompt token 序列, 得到 inputs_embeds。
        forward / compute_response_logprobs / GRPO 生成 三处共用。
        """
        ...
        if (samples['input_ids'][0] == self.MULTI_PATCH_TOKEN_ID).sum() != 0: # 这时才需要 multi
    ...
    def forward(self, samples):
        """
        inputs_embeds:  [<bos>###Human: <Video> <ImageHere>*32 </Video> xxx###Assistant: {target}<eos=pad><pad>...]
        attention_mask: [1, 1, ...                                                             1,  0,  0, ...]
        targets:        [-100......,                                                           ..., -100, {target}<eos=pad>, -100,..]
        """
        inputs_embeds = self.prepare_inputs_embeds(samples)
    ...
    def compute_response_logprobs(self, samples, return_tokens=False):
        """
        计算每个样本 answer 部分 (labels != IGNORE_INDEX) 的对数似然, 用于 GRPO。
        返回 (return_tokens=False):
          seq_logprobs: [B], 每个样本 answer 部分 logprob 之和
          n_tokens:     [B], 每个样本 answer 部分 token 数 (不含 -100)
        返回 (return_tokens=True):
          token_logprobs: [B, L-1], 逐 token 对数似然
          mask:           [B, L-1], answer 部分 mask (bool)
        """
        inputs_embeds = self.prepare_inputs_embeds(samples)
        with self.maybe_autocast():
            outputs = self.llama_model(
        ...
        logits = logits[:, :-1, :].float()                         # 预测下一个 token
        labels = samples['labels'][:, 1:]                          # [B, L-1]
        log_probs = torch.log_softmax(logits, dim=-1)
        token_logprobs = log_probs.gather(-1, labels.clamp(min=0).unsqueeze(-1)).squeeze(-1)  # [B, L-1]
        mask = labels != config.IGNORE_INDEX                       # answer 部分
        if return_tokens:
            return token_logprobs, mask
        seq_logprobs = (token_logprobs * mask).sum(dim=-1)         # [B]
        n_tokens = mask.sum(dim=-1)                                # [B]
        return seq_logprobs, n_tokens
